...
make. `status` is encoded with the cause of death.
...
EDA often tells us about missing data. 
...
and what order we will impute variables. MICE chooses chooses
...
with missing values and any extra info about these missing values. 
Notice we removed `patno` and `sdate` before using `mice()`.
process. Don't include useless columns like PatientNumber. 
This will only slow down or worsen the imputation.
 `m` is the number of imputations to generate. `method = 'pmm'` tells MICE
...
```{r}
nonlinear.fit <- cph(Surv(dtime, status) ~ rx + rcs(age,3), data = prostate, x=T, y=T)
nonlinear.fit
```
...
is associated with survival, not individual age parameters
...
sapmles. Finding a significant hazard ratio in one cohort but not
...
separate subgroups groups. Interactions often require 16x the number of
...
the final hazard ratio for each subgroup, we sum any related coefficients.

Our Cox regression takes the following form
...
5.0 mg estrogen, and $x_4$ indicates bone metastasis. $\beta_5$, $\beta_6$, and $\beta_7$
represent the interaction terms. To compute the full hazard 
for bone metastasis ($x_4 = 1$) patients treated with 5.0 mg 
estrogen ($x_3 = 1$ and $x_1, x_2 = 0$) we'd add

$$
e^{\beta_3 + \beta_4 + \beta_7} = \text{HR}\\
e^{0.1112 + 1.0804 + -0.8701} = 1.379195
$$
...
ratio for stays constant over time. 
...
Cox may not be appropriate for our analysis. Accelerated
time failure (AFT) models may better suited for the data.
See RMS Chapter 18 for information on parametric AFT models.
...
van Burren for more information. 
