...
The presentation covered statistical
...
loading data, and manipulating dataframes. It's also
important to understand introductory level statistics
...
In this lesson we'll analyze survival data from
a Randomized Clinical Trial of estrogen therapy for
...
Before we begin, remember that statistics can be complicated
and it's always good to ask for help from a statistician!
Feel free to send me an email at 
[tjbencomo@gmail.com](tjbencomo@gmail.com) with questions. I'd be happy
to help connect you with the right resources for your
research!
...
us easily manipulate dataframes. `ggplot2` makes data
visualization easy. `rpart`, `mice`, and `tidyr` help us deal
...
between variables, and checking for outliers.
See Grolemund and Wickham's 
...
handle either dead or censored. 
Let's treat all causes of death as equal and encode 
status so 1 means death and 0 means censored. Careful
...
Changing `status`'s encoding is the only fix we need to make. In the real
world, data is almost never this neat. Be careful to check that everything
...
Check what the reference level is for each factor.
...
as we'll want to compare estrogen with the placebo. Use the `relevel()` command
...
For 2), we can model missing data using MICE. We tell MICE which variables
...
an added $e^{0.0322} =$ `r exp(0.0322)` of hazard.

The arguments `x=T, y=T` tells `R` to store the dataset
for later use plotting predictor relationships.
...
statistic calculated from the model's maximum likelihood estimation.
...
(`age` and `age'`, `age''`, and `age'''`). This makes interpretting our 
...
is associated with survival, not individual age parameters. Do
...
terms. It's equivalent to writing `x1 + x2 + x1*x2`. We can look the p-values
...
separate, smaller subgroups. Interactions often require 16x the number of
...
Cox may not be appropriate for your analysis. Accelerated
time failure (AFT) models may be more appropriate.
